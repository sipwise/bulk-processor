...
sub _reader {
    ...
    eval {
        ...
        &{$context->{initcontext_code}}($context) if 'CODE' eq ref $context->{initcontext_code};

        local *INPUTFILE;
        if (not open (INPUTFILE, '<:encoding(' . $encoding . ')', $file)) {
        ...
        while (($state & $RUNNING) == $RUNNING and ($state & $ERROR) == 0) { #as long there is one running consumer and no defunct consumer
            my @lines = ();
            while ((scalar @lines) < $context->{blocksize} and defined ($n = read(INPUTFILE,$chunk,$buffersize)) and $n != 0) {
                $buffer .= $chunk;
                &{$context->{extractlines_code}}(\$buffer,\@lines);
            }
            if (not defined $n) {
                ...
                if ($n == 0 && length($buffer) > 0) {
                    push(@lines,$buffer);
                }
                my @rowblock :shared = ();
                foreach my $line (@lines) {
                    push(@rowblock,&{$context->{extractfields_code}}($context,$line));
                }
                my $realblocksize = scalar @rowblock;
                my %packet :shared = ();
