...
sub set_time {
	...
	if (defined $o) {
		...
		faketimeinfo("applying fake time offset '$o' - current time: " . datetime_to_string($now),$logger);
	} else {
		_set_fake_time();
		my $now = _current_local();
		faketimeinfo("resetting fake time - current time: " . datetime_to_string($now),$logger);
	}
}
	...
	if (defined $o) {
		if (ref $o eq 'DateTime') {
			...
			if (!$o) {
				...
				faketimeerror("Invalid time offset: '$o'",$logger);
			}
		}
		Time::Warp::to($o);
	} else {
