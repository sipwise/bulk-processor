...
use NGCP::BulkProcessor::Dao::mr553::provisioning::voip_allowed_ip_groups qw();
use NGCP::BulkProcessor::Dao::mr553::provisioning::voip_preferences qw();
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            if ($record->{attribute} eq $NGCP::BulkProcessor::Dao::mr553::provisioning::voip_preferences::ALLOWED_IPS_GRP_ATTRIBUTE) {
                my @allowed_ip_groups : shared = map { $_->{ipnet}; } @{NGCP::BulkProcessor::Dao::mr553::provisioning::voip_allowed_ip_groups::source_findby_group_id(
