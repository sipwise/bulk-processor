package NGCP::BulkProcessor::Dao::mr553::provisioning::voip_cf_time_sets;
use strict;
...
use NGCP::BulkProcessor::Dao::mr553::provisioning::voip_cf_periods qw();
...
my $tablename = 'voip_cf_time_sets';
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            # transformations go here ...
            $record->{periods} = NGCP::BulkProcessor::Dao::mr553::provisioning::voip_cf_periods::source_findby_timesetid($source_dbs,$record->{id});
