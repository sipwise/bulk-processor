package NGCP::BulkProcessor::Dao::mr553::billing::voip_subscribers;
use strict;
...
use NGCP::BulkProcessor::Dao::mr553::provisioning::voip_subscribers qw();
use NGCP::BulkProcessor::Dao::mr553::billing::voip_numbers qw();
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            $record->{provisioning_voip_subscriber} = NGCP::BulkProcessor::Dao::mr553::provisioning::voip_subscribers::source_findby_uuid($source_dbs,$record->{uuid});
            $record->{voip_numbers} = NGCP::BulkProcessor::Dao::mr553::billing::voip_numbers::source_findby_subscriberid($source_dbs,$record->{id});
            $record->{primary_number} = NGCP::BulkProcessor::Dao::mr553::billing::voip_numbers::source_findby_id($source_dbs,$record->{primary_number_id});

            if ($record->{contact_id}) {
                $record->{contact} = NGCP::BulkProcessor::Dao::mr553::billing::contacts::source_findby_id($source_dbs,$record->{contact_id});
            } else {
                $record->{contact} = undef;
            }
