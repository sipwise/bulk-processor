package NGCP::BulkProcessor::Dao::mr341::provisioning::voip_cf_destination_sets;
use strict;
...
use NGCP::BulkProcessor::Dao::mr341::provisioning::voip_cf_destinations qw();
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            # transformations go here ...
            $record->{destinations} = NGCP::BulkProcessor::Dao::mr341::provisioning::voip_cf_destinations::source_findby_destinationsetid($source_dbs,$record->{id});
