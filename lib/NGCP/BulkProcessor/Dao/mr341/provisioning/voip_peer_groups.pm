package NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_groups;
use strict;
...
use NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_hosts qw();
use NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_rules qw();
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            $record->{contract} = NGCP::BulkProcessor::Dao::mr341::billing::contracts::source_findby_id($source_dbs,$record->{peering_contract_id});

            $record->{hosts} = NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_hosts::source_findby_groupid($source_dbs,$record->{id});
            $record->{rules} = NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_rules::source_findby_groupid($source_dbs,$record->{id});
