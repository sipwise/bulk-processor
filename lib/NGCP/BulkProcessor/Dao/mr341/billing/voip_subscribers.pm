package NGCP::BulkProcessor::Dao::mr341::billing::voip_subscribers;
use strict;
...
use NGCP::BulkProcessor::Dao::mr341::provisioning::voip_subscribers qw();
use NGCP::BulkProcessor::Dao::mr341::billing::voip_numbers qw();
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            $record->{voip_numbers} = NGCP::BulkProcessor::Dao::mr341::billing::voip_numbers::source_findby_subscriberid($source_dbs,$record->{id});
            $record->{primary_number} = NGCP::BulkProcessor::Dao::mr341::billing::voip_numbers::source_findby_id($source_dbs,$record->{primary_number_id});
