...
use NGCP::BulkProcessor::Dao::mr102::billing::ncos_levels qw();
...
sub source_buildrecords_fromrows {
    ...
    if (defined $rows and ref $rows eq 'ARRAY') {
        foreach my $row (@$rows) {
            ...
            if ($record->{attribute} eq $NGCP::BulkProcessor::Dao::mr102::provisioning::voip_preferences::NCOS_ID_ATTRIBUTE) {
                 my %ncos : shared = ( %{NGCP::BulkProcessor::Dao::mr102::billing::ncos_levels::source_getuniquename($source_dbs,$record->{value})} );
                 $record->{ncos} = \%ncos;
                 #delete $record->{value};
            }
