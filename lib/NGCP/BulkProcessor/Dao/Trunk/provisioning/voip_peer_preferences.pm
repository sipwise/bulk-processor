package NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_peer_preferences;
use strict;
...
our @EXPORT_OK = qw(
    ...
    delete_preferences
    findby_peerhostid_attributeid
    countby_peerhostid_attributeid_value

    $TRUE
    $FALSE
);

my $tablename = 'voip_peer_preferences';
...
sub findby_peerhostid_attributeid {

    my ($xa_db,$peer_host_id,$attribute_id,$load_recursive) = @_;
    ...
    my $stmt = 'SELECT * FROM ' . $table . ' WHERE ' .
            $db->columnidentifier('peer_host_id') . ' = ?';
    my @params = ($peer_host_id);
...
sub countby_peerhostid_attributeid_value {

    my ($peer_host_id,$attribute_id,$value) = @_;
    ...
    my @terms = ();
    if ($peer_host_id) {
        push(@terms,$db->columnidentifier('peer_host_id') . ' = ?');
        push(@params,$peer_host_id);
    }
    if ($attribute_id) {
...
sub delete_preferences {
    ...
    my ($xa_db,$peer_host_id,$attribute_id,$vals) = @_;
