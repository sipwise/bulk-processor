...
use NGCP::BulkProcessor::Utils qw(makedir cleanupdir);
...
sub setup {
    ...
    if ($self->{crtauth}) {
        makedir(_get_api_cert_dir(),\&fileerror,getlogger(__PACKAGE__));
        $self->{crt_path} = sprintf($API_CERT_FILENAME_FORMAT,_get_api_cert_dir(),$self->{username}=~s/[^a-z0-9 _]/replacement/gir);
    } else {
        $self->{crt_path} = undef;
    }
