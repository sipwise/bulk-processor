...
sub checktableinfo {
    ...
    if (not exists $table_fieldnames_cached->{$tid}->{$connectidentifier}->{$class}) {
        ...
        #$table_fieldnames_cached->{$tid}->{$connectidentifier}->{$class} = shared_clone($db->getfieldnames($tablename));
        $table_fieldnames_cached->{$tid}->{$connectidentifier}->{$class} = $db->getfieldnames($tablename);
        #my $fieldnames = $db->getfieldnames($tablename);
        if (!defined $table_expected_fieldnames->{$tid}->{$connectidentifier}->{$class}
            or (scalar @{$table_expected_fieldnames->{$tid}->{$connectidentifier}->{$class}}) == 0
            or setcontains($table_expected_fieldnames->{$tid}->{$connectidentifier}->{$class},$table_fieldnames_cached->{$tid}->{$connectidentifier}->{$class},1)) {
            ...
            fieldnamesdiffer($db,$tablename,$table_expected_fieldnames->{$tid}->{$connectidentifier}->{$class},$table_fieldnames_cached->{$tid}->{$connectidentifier}->{$class},getlogger(__PACKAGE__));
            $result = 0;
        }
    }
