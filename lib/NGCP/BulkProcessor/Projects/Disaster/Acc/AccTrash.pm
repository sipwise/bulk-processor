    ...
    return ($result && NGCP::BulkProcessor::Dao::Trunk::kamailio::acc_trash::process_records(
        ...
        process_code => sub {
            ...
            if ($context->{sleep_secs} > 0.0 and $context->{acc_record_limit} > 0) {
                while (((my $acc_count = NGCP::BulkProcessor::Dao::Trunk::kamailio::acc::count($context->{db})) + scalar @$records) > $context->{acc_record_limit}) {
                    _info($context,"$acc_count acc records (limit $context->{acc_record_limit}), sleep for $context->{sleep_secs} secs ...");
                    sleep($context->{sleep_secs});
                }
            }
...
sub _process_acc_trash {
    ...
    eval {
        ...
        foreach my $acc_trash (@{$context->{acc_trash}}) {
            ...
            unless ($incomplete) {
                NGCP::BulkProcessor::Dao::Trunk::kamailio::acc::insert_row($context->{db},$acc_trash);
                _info($context,"($context->{rownum}) " . 'acc trash record id ' . $acc_trash->{id} . " copied",1);
            }
        }
        unless ($incomplete) {
            NGCP::BulkProcessor::Dao::Trunk::kamailio::acc_trash::delete_callids($context->{db},[ $context->{call_id} ]);
...
sub _reset_process_acc_trash_context {
    ...
    $context->{acc_trash} = NGCP::BulkProcessor::Dao::Trunk::kamailio::acc_trash::findby_callids($context->{db},[ $context->{call_id} ]);
