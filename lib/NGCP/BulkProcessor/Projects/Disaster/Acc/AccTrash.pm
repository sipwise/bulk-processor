...
sub _process_acc_trash {
    ...
    if ($err) {
        ...
        if ($skip_errors) {
            _warn($context,"($context->{rownum}) " . 'database error with acc trash id ' . $context->{acc_trash}->{id} . ': ' . $err);
        } else {
            _error($context,"($context->{rownum}) " . 'database error with acc trash id ' . $context->{acc_trash}->{id} . ': ' . $err);
        }
    }
}
