...
sub _fix_free_cash {
    ...
    eval {
        ...
        my $contract = NGCP::BulkProcessor::Dao::Trunk::billing::contracts::forupdate_id($context->{db},$contract_id);
        $context->{contract} = $contract;
        my $actual_profile = $context->{actual_profile};

        my @balances = sort NGCP::BulkProcessor::Dao::Trunk::billing::contract_balances::sort_by_end_desc
        ...
        if ($mapping) {
            ...
            NGCP::BulkProcessor::Dao::Trunk::billing::contract_balances::update_row($context->{db},{
                id => $balance->{id},
                cash_balance => ($balance->{cash_balance} + $delta),
            });

            NGCP::BulkProcessor::Dao::Trunk::billing::topup_log::insert_row($context->{db},{
                timestamp => $context->{now}->epoch(),
                type => $NGCP::BulkProcessor::Dao::Trunk::billing::topup_log::SET_BALANCE_TYPE,
                outcome => $NGCP::BulkProcessor::Dao::Trunk::billing::topup_log::OK_OUTCOME,
            ...
            _info($context,"($context->{rownum}) " . 'contract id ' . $contract_id . ' cash balance delta: ' . $delta);
...
sub _fix_free_cash_checks {
    ...
    my $profile_count = 0;
    eval {
        ($context->{billing_profile_map},my $ids,my $profiles) = array_to_map(NGCP::BulkProcessor::Dao::Trunk::billing::billing_profiles::findall(),
    ...
    my $package_count = 0;
    eval {
        ($context->{profile_package_map},my $ids,my $packages) = array_to_map(NGCP::BulkProcessor::Dao::Trunk::billing::profile_packages::findall(),
            sub { return shift->{id}; }, sub { return shift; }, 'first' );
...
sub _reset_fix_free_cash_context {
    ...
    my $contract = NGCP::BulkProcessor::Dao::Trunk::billing::contracts::findby_id($contract_id);
    ...
    if ($mapping) {
        ...
        if (($actual_profile->{interval_free_cash} // 0.0) != 0.0) {
            my $carry_over_mode = $NGCP::BulkProcessor::Dao::Trunk::billing::profile_packages::DEFAULT_CARRY_OVER_MODE;
