...
eval {
    ...
    foreach my $contract_balance (sort NGCP::BulkProcessor::Dao::Trunk::billing::contract_balances::sort_by_end @{$context->{contract_balances}}) {
        print "  " . $contract_balance->{_start} . ' ' . $contract_balance->{_end} . "\n";
        if (defined $last_end) {
            my $date_comparison = DateTime->compare($last_end->clone->add(seconds => 1), $contract_balance->{_start});
        ...
        $last_end = $contract_balance->{_end};
    }
};
if ($@) {
print $@;
