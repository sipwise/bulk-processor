...
sub _reset_fix_free_cash_context {
    ...
    foreach my $balance (sort NGCP::BulkProcessor::Dao::Trunk::billing::contract_balances::sort_by_end_desc
        ...
        $balance->{_topups} = NGCP::BulkProcessor::Dao::Trunk::billing::topup_log::findby_contractbalanceid(
            $balance->{id},undef);

        my $topup_sum = 0.0;
        foreach my $topup_log (@{$balance->{_topups}}) {
            $topup_sum += $topup_log->{amount} if (defined $topup_log->{amount} and $topup_log->{outcome} eq $NGCP::BulkProcessor::Dao::Trunk::billing::OK_OUTCOME;
        }
