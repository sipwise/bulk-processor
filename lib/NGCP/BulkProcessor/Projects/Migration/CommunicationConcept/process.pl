...
my $copy_peers_mr341_task_opt = 'copy_peers_mr341';
push(@TASK_OPTS,$copy_peers_mr341_task_opt);
...
sub main() {
    ...
    if (defined $tasks and 'ARRAY' eq ref $tasks and (scalar @$tasks) > 0) {
        ...
        foreach my $task (@$tasks) {
            ...
            } elsif (lc($copy_peers_mr341_task_opt) eq lc($task)) {
                if (taskinfo($copy_peers_mr341_task_opt,$result,1)) {
                    next unless check_dry();
                    $result &= copy_peers_mr341_task(\@messages);
                    $completion |= 1;
                }
...
sub copy_peers_mr341_task {

    my ($messages) = @_;
    my ($result,$warning_count) = (0,0);
    eval {
        ($result,$warning_count) = copy_peers_mr341();
    };
    my $err = $@;
    my $stats = ": $warning_count warnings";
    eval {
