...
use NGCP::BulkProcessor::Projects::Migration::CommunicationConcept::Provisioning qw(
    copy_contracts
);
...
my $copy_contracts_task_opt = 'copy_contracts';
push(@TASK_OPTS,$copy_contracts_task_opt);
...
sub main() {
    ...
    if (defined $tasks and 'ARRAY' eq ref $tasks and (scalar @$tasks) > 0) {
        ...
        foreach my $task (@$tasks) {
            ...
            } elsif (lc($copy_contracts_task_opt) eq lc($task)) {
                if (taskinfo($copy_contracts_task_opt,$result,1)) {
                    next unless check_dry();
                    $result &= copy_contracts_task(\@messages);
                    $completion |= 1;
                }
...
sub copy_contracts_task {

    my ($messages) = @_;
    my ($result,$warning_count) = (0,0);
    eval {
        ($result,$warning_count) = copy_contracts();
    };
    my $err = $@;
    ...
    if ($err or !$result) {
        push(@$messages,"copying contracts INCOMPLETE$stats");
    } else {
        push(@$messages,"copying contracts completed$stats");
    }
