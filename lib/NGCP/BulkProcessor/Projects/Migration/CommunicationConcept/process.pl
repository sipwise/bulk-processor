...
use NGCP::BulkProcessor::Projects::Migration::CommunicationConcept::System qw(
    copy_resellers
    copy_peers
);
...
my $copy_resellers_task_opt = 'copy_resellers';
push(@TASK_OPTS,$copy_resellers_task_opt);

my $copy_peers_task_opt = 'copy_peers';
push(@TASK_OPTS,$copy_peers_task_opt);
...
sub main() {
    ...
    if (defined $tasks and 'ARRAY' eq ref $tasks and (scalar @$tasks) > 0) {
        ...
        foreach my $task (@$tasks) {
            ...
            } elsif (lc($copy_resellers_task_opt) eq lc($task)) {
                if (taskinfo($copy_resellers_task_opt,$result,1)) {
                    next unless check_dry();
                    $result &= copy_resellers_task(\@messages);
                    $completion |= 1;
                }
            } elsif (lc($copy_peers_task_opt) eq lc($task)) {
                if (taskinfo($copy_peers_task_opt,$result,1)) {
                    next unless check_dry();
                    $result &= copy_peers_task(\@messages);
                    $completion |= 1;
                }
...
sub copy_resellers_task {

    my ($messages) = @_;
    my ($result,$warning_count) = (0,0);
    eval {
        ($result,$warning_count) = copy_resellers();
    };
    my $err = $@;
...
sub copy_peers_task {
    ...
    my ($result,$warning_count) = (0,0);
    eval {
        ($result,$warning_count) = copy_peers();
    };
    my $err = $@;
    my $stats = ": $warning_count warnings";
    eval {
    ...
    if ($err or !$result) {
        push(@$messages,"copying peers INCOMPLETE$stats");
    } else {
        push(@$messages,"copying peers completed$stats");
    }
    destroy_all_dbs(); #every task should leave with closed connections.
    return $result;
