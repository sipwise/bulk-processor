...
sub _copy_contract {
    ...
    eval {
        ...
        if ((scalar @existing_billing_voip_subscribers) > 0) {
            _warn($context,"contract with subscribers " . join(',',map { $_->{username}; } @existing_billing_voip_subscribers) . " already exists, skipping");
        } else {
    ...
    eval {
        foreach my $old_reseller (@{run_dao_method('billing::resellers::source_findall',$context->{source_dbs})}) {
            ...
            foreach my $old_domain (@{$old_reseller->{domains}}) {
                my $new_domain = NGCP::BulkProcessor::Dao::Trunk::billing::domains::findby_domain($old_domain->{domain});
                die("domain '$old_domain->{domain}' not found") unless $new_domain;
                $context->{domain_id_map}->{$old_domain->{id}} = $new_domain->{id};
            }
...
sub _create_contact {
    ...
    if ((scalar @$existing_contacts) > 0) {
        ...
        if ((scalar @$existing_contacts) > 1) {
            ...
            _info($context,"existing customer contact id $existing_contact->{id} found",0);
        }
        ...
        _info($context,"system customer id $context->{contract}->{contact_id} created",0);
    }
...
sub _create_contract {
    ...
    _info($context,"customer contract id $context->{contract_id} created",0);
    ...
    _info($context,"customer contract id $context->{contract_id} contract balances created",0);
...
sub _append_billing_mappings {
    ...
    _info($context,"customer contract id $contract_id billing mappings created");
}
...
sub _copy_contract_init_context {
    ...
    $result = 0 unless scalar @{$context->{voip_subscribers}};
