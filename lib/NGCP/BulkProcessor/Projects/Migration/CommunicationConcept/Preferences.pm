...
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_peer_preferences qw();
...
my %get_preference_sub_names = (
    ...
    voip_peer_preferences => 'findby_peerhostid_attributeid',
);
...
sub create_peer_preferences {
    my ($context, $peer_host, $ps) = @_;
    return _create_preferences($context,'voip_peer_preferences','peer_pref',$ps,{
        id => $peer_host->{id},
    });
}
