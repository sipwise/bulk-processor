...
sub copy_peers_mr341 {
    ...
    if (_copy_peer_checks($context)) {
        ...
        foreach my $peer_group (@{NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_groups::source_findall($context->{source_dbs})}) {
...
sub _copy_peer_checks {
    ...
    eval {
        $context->{pstn_peering_account_product} = NGCP::BulkProcessor::Dao::Trunk::billing::products::findby_resellerid_handle(undef,
            $NGCP::BulkProcessor::Dao::Trunk::billing::products::PSTN_PEERING_ACCOUNT_HANDLE)->[0];
    };
    if ($@ or not defined $context->{pstn_peering_account_product}) {
        _error($context,"cannot find $NGCP::BulkProcessor::Dao::Trunk::billing::products::PSTN_PEERING_ACCOUNT_HANDLE product");
        $result = 0; #even in skip-error mode..
    } else {
        _info($context,"$NGCP::BulkProcessor::Dao::Trunk::billing::products::PSTN_PEERING_ACCOUNT_HANDLE product found");
    }
...
sub _copy_billing_mappings {
    ...
    eval {
        ...
        foreach my $contract_id (keys %{$context->{deferred_billing_mappings}}) {
            my @mappings = ();
            my $contract = $context->{deferred_billing_mappings}->{$contract_id}->{contract};
            foreach my $bm (@{sort_by_configs($context->{deferred_billing_mappings}->{$contract_id}->{billing_mappings},[
                    {   numeric     => 1,
                        dir         => 1, #-1,
                        memberchain => [ 'id' ],
                    }
...
sub _copy_billing_fees_mr341 {
    ...
    foreach my $old_billing_profile_id (keys %{$reseller_context->{billing_profile_id_map}}) {
        ...
        if (_copy_billing_zones_mr341($context)) {
            undef $context->{db};
            destroy_all_dbs();
