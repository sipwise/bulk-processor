...
use NGCP::BulkProcessor::Dao::mr341::provisioning::voip_dom_preferences qw();
use NGCP::BulkProcessor::Dao::mr341::provisioning::voip_peer_preferences qw();

use NGCP::BulkProcessor::Dao::mr553::billing::resellers qw();
use NGCP::BulkProcessor::Dao::mr553::billing::billing_zones qw();
use NGCP::BulkProcessor::Dao::mr553::billing::billing_fees qw();
use NGCP::BulkProcessor::Dao::mr553::provisioning::voip_peer_groups qw();
use NGCP::BulkProcessor::Dao::mr553::provisioning::voip_dom_preferences qw();
...
use NGCP::BulkProcessor::Dao::Trunk::billing::ncos_pattern_list qw();
...
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_peer_inbound_rules qw();
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_rewrite_rule_sets qw();
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_rewrite_rules qw();
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_sound_sets qw();
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_sound_files qw();
use NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_sound_handles qw();
...
sub _init_copy_peer_context {
    ...
    $context->{old_peer_group} = $peer_group;
...
sub _copy_peer_checks {
    ...
    eval {
        foreach my $attribute (@{run_dao_method('provisioning::voip_peer_preferences::source_findby_attributesused',$context->{source_dbs})}) {
...
sub copy_resellers {
    ...
    if (_copy_reseller_checks($context)) {
        ...
        $result &= _copy_billing_mappings($context);
        $result &= _copy_domain_prefs($context);
...
sub _copy_reseller {
    ...
    eval {
        ...
        if ((scalar @$existing_resellers) > 0) {
            ...
            _create_domains($context);
            _create_email_templates($context);
            _create_ncos_levels($context);
            _create_rewrite_rule_sets($context);
            _create_sound_sets($context);
        ...
        if ($dry) {
            ...
            if ($result) {
                ...
                @{$context->{domain_id_prefs}}{keys %{$context->{_domain_id_prefs}}} = values %{$context->{_domain_id_prefs}};
            }
        }
...
sub _copy_billing_mappings {
    ...
    my $context = {
        db => $reseller_context->{db},
        #source_dbs => $reseller_context->{source_dbs},
        deferred_billing_mappings => $reseller_context->{deferred_billing_mappings},
        billing_profile_id_map => $reseller_context->{billing_profile_id_map},
        default_billing_profile => $reseller_context->{default_billing_profile},
    };
...
sub _create_peer_hosts {
    ...
    foreach my $h (@{$context->{hosts}}) {
        my $host = { %$h };
        delete $host->{id};
        my $preferences = delete $host->{peer_preferences};
        ...
        unless (create_peer_preferences($context,$host,$preferences)) {
            $result = 0;
        }
...
sub _create_peer_inbound_rules {
    ...
    foreach my $ir (@{$context->{inbound_rules}}) {
        ...
        $inbound_rule->{id} = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_peer_inbound_rules::insert_row($context->{db},
            $inbound_rule,
        );
...
sub _create_domains {
    ...
    foreach my $d (@{$context->{domains}}) {
        my $domain = { %$d };
        delete $domain->{id};
        my $prefs = delete $domain->{voip_dom_preferences};
        ...
        $context->{_domain_id_prefs}->{NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_domains::insert_row($context->{db},
...
sub _create_ncos_levels {

    my ($context) = @_;

    foreach my $nl (@{$context->{ncos_levels}}) {
        my $ncos_level = { %$nl };
        my $pl = delete $ncos_level->{ncos_pattern_list};
        delete $ncos_level->{id};
        $ncos_level->{reseller_id} = $context->{reseller_id};
        $ncos_level->{id} = NGCP::BulkProcessor::Dao::Trunk::billing::ncos_levels::insert_row($context->{db},
            $ncos_level,
        );
        foreach my $p (@$pl) {
            my $pattern = { %$p };
            $pattern->{ncos_level_id} = $ncos_level->{id};
            delete $pattern->{id};
            $pattern->{id} = NGCP::BulkProcessor::Dao::Trunk::billing::ncos_pattern_list::insert_row($context->{db},
                $pattern,
            );
        }
        _info($context,"ncos level id $ncos_level->{id} '$ncos_level->{level}' created",0);
    }
...
sub _create_rewrite_rule_sets {

    my ($context) = @_;

    foreach my $rwrs (@{$context->{rewrite_rule_sets}}) {
        my $rewrite_rule_set = { %$rwrs };
        my $rrs = delete $rewrite_rule_set->{rewrite_rules};
        delete $rewrite_rule_set->{id};
        map { delete $rewrite_rule_set->{$_}; } @NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_rewrite_rule_sets::DPID_FIELDS;
        #delete $rewrite_rule_set->{caller_in_dpid};
        #delete $rewrite_rule_set->{callee_in_dpid};
        #delete $rewrite_rule_set->{caller_out_dpid};
        #delete $rewrite_rule_set->{callee_out_dpid};
        #delete $rewrite_rule_set->{caller_lnp_dpid};
        #delete $rewrite_rule_set->{callee_lnp_dpid};
        $rewrite_rule_set->{reseller_id} = $context->{reseller_id};
        $rewrite_rule_set->{id} = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_rewrite_rule_sets::insert_row($context->{db},
            $rewrite_rule_set,
        );
        foreach my $rr (@$rrs) {
            my $rewrite_rule = { %$rr };
            $rewrite_rule->{set_id} = $rewrite_rule_set->{id};
            delete $rewrite_rule->{id};
            $rewrite_rule->{id} = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_rewrite_rules::insert_row($context->{db},
                $rewrite_rule,
            );
        }
        _info($context,"rewrite rule set id $rewrite_rule_set->{id} '$rewrite_rule_set->{name}' created",0);
    }
...
sub _create_sound_sets {

    my ($context) = @_;

    my $result = 1;

    foreach my $ss (@{$context->{sound_sets}}) {
        my $sound_set = { %$ss };
        my $sfs = delete $sound_set->{sound_files};
        delete $sound_set->{id};
        $sound_set->{reseller_id} = $context->{reseller_id};
        $sound_set->{id} = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_sound_sets::insert_row($context->{db},
            $sound_set,
        );
        foreach my $sf (@$sfs) {
            my $sound_file = { %$sf };
            $sound_file->{set_id} = $sound_set->{id};
            delete $sound_file->{id};
            my $group = delete $sound_file->{group};
            my $handle = delete $sound_file->{handle};
            if ($group and $handle) {
                if (exists $context->{sound_group_handle_map}->{$group}
                    and exists $context->{sound_group_handle_map}->{$group}->{$handle}) {
                    $sound_file->{handle_id} = $context->{sound_group_handle_map}->{$group}->{$handle}->{id};
                } else {
                    _warn($context,"unknown sound group $group handle $handle");
                    $result = 0;
                    next;
                }
            } else {
                $sound_file->{handle_id} = undef;
            }
            $sound_file->{id} = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_sound_files::insert_row($context->{db},
                $sound_file,
            );
        }
        _info($context,"system sound set id $sound_set->{id} '$sound_set->{name}' created",0);
    }
...
sub _init_copy_reseller_context {
    ...
    $context->{_billing_profile_id_map} = {};
    $context->{domain_id_prefs} //= {};
    $context->{_domain_id_prefs} = {};

    #$context->{reseller} = dclone($reseller);
    $context->{old_reseller} = $reseller;
    ...
    $context->{ncos_levels} = delete $context->{reseller}->{ncos_levels};
    $context->{rewrite_rule_sets} = delete $context->{reseller}->{rewrite_rule_sets};
    $context->{sound_sets} = delete $context->{reseller}->{sound_sets};
    delete $context->{contract}->{id};
...
sub _copy_reseller_checks {
    ...
    eval {
        ($context->{sound_group_handle_map},my $groups,my $handlegroups) = array_to_map(NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_sound_handles::findby_namegroup(),sub {
            return shift->{group};
        }, sub {
            return shift;
        },'group');
        foreach my $group (@$groups) {
            ($context->{sound_group_handle_map}->{$group}, my $handles, my $sound_handles) = array_to_map($context->{sound_group_handle_map}->{$group},sub {
                return shift->{name};
            }, sub {
                return shift;
            },'first');
    ...
    if ($@) {
        _error($context,"failed to map sound groups/handles");
        $result = 0; #even in skip-error mode..
    } else {
        _info($context,(scalar keys %{$context->{sound_group_handle_map}}) . " sound groups mapped");
    }
...
sub _copy_domain_prefs {
    ...
    my $context = {
        cleanup_aig_sequence => 0,
        source_dbs => $reseller_context->{source_dbs},
        db => $reseller_context->{db},
        domain_id_prefs => $reseller_context->{domain_id_prefs},
    };
    if (_copy_domain_prefs_checks($context)) {
        foreach my $prov_domain_id (keys %{$context->{domain_id_prefs}}) {
            my $preferences = $context->{domain_id_prefs}->{$prov_domain_id};
            eval {
                $context->{db}->db_begin();
                if (create_dom_preferences($context,{ id => $prov_domain_id },$preferences)) {
                    _info($context,"prov domain id $prov_domain_id preferences created");
                } else {
                    $result = 0;
                }
                $context->{db}->db_commit();
...
sub _copy_domain_prefs_checks {
    ...
    eval {
        foreach my $attribute (@{run_dao_method('provisioning::voip_dom_preferences::source_findby_attributesused',$context->{source_dbs})}) {
            (my $obsolete, my $replacements) = check_replaced_prefs($attribute);
            if ($obsolete) {
                foreach my $replacement (@$replacements) {
                    my $preferrence = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_preferences::findby_attribute($replacement);
...
sub _error {
    ...
    if ($context->{old_reseller}) {
        $message = 'source reseller id ' . $context->{old_reseller}->{id} . ': ' . $message;
    } elsif ($context->{old_peer_group}) {
        $message = 'source peer group id ' . $context->{old_peer_group}->{id} . ': ' . $message;
    }
