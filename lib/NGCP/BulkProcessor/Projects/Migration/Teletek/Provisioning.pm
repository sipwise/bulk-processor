...
sub _create_aliases {
    ...
    if ((scalar @{$context->{numbers}->{other}}) > 0) {
        ...
        #my @voip_numbers_other = ();
        my @voip_number_ids = ();
        my @usernames = ();
        #my @allowed_clis = @{$context->{preferences}->{allowed_clis}};
        ...
        foreach my $number (@{$context->{numbers}->{other}}) {
            ...
            if (defined $voip_number) {
                ...
                $voip_number = NGCP::BulkProcessor::Dao::Trunk::billing::voip_numbers::insert_row($context->{db},
                    cc => $number->{cc},
                    ac => $number->{ac},
                    sn => $number->{sn},
                    reseller_id => $context->{reseller}->{id},
