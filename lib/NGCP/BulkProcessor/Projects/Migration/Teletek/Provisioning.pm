...
sub _provision_susbcriber_init_context {
    ...
    my @allowed_ips = ();
    if (defined $first->{allowed_ips} and length($first->{allowed_ips}) > 0) {
        foreach my $ipnet (map { local $_ = $_; trim($_); } split(/$split_ipnets_pattern/,$first->{allowed_ips})) {
            if (check_ipnet($ipnet)) {
                if ('0.0.0.0' ne $ipnet) {
                    push(@allowed_ips,$ipnet);
                } else {
