...
sub create_subscriber_task {
    ...
    eval {
        ...
        $registration_count = NGCP::BulkProcessor::Dao::Trunk::kamailio::location::countby_usernamedomain(undef,undef);
        $stats .= "\n  registrations: " . $registration_count . ' rows';
    ...
    if ($err or !$result) {
        ...
        if (not $dry and $registration_count > 0) {
            push(@$messages,"THERE WERE $registration_count LOCATIONS RECORDS ADDED. YOU MIGHT WANT TO RESTART KAMAILIO NOW ...");
        }
    }
    destroy_all_dbs(); #every task should leave with closed connections.
    return $result;
