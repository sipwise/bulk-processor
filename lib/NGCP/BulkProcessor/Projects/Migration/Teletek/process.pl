...
my $import_flatfile_task_opt = 'import_flatfile';
push(@TASK_OPTS,$import_flatfile_task_opt);
my $import_truncate_flatfile_task_opt = 'truncate_flatfile';
push(@TASK_OPTS,$import_truncate_flatfile_task_opt);
...
sub main() {
    ...
    if (defined $tasks and 'ARRAY' eq ref $tasks and (scalar @$tasks) > 0) {
        ...
        foreach my $task (@$tasks) {
            ...
            } elsif (lc($import_flatfile_task_opt) eq lc($task)) {
                $result &= import_flatfile_task(\@messages) if taskinfo($import_flatfile_task_opt,$result);
            } elsif (lc($import_truncate_flatfile_task_opt) eq lc($task)) {
                $result &= import_truncate_flatfile_task(\@messages) if taskinfo($import_truncate_flatfile_task_opt,$result);
...
sub import_flatfile_task {

    my ($messages) = @_;
    my ($result,$warning_count) = (0,0);
    eval {
        ($result,$warning_count) = import_flatfile($flatfile_filename);
    };
    #my $err = $@;
    #my $stats = ($skip_errors ? ": $warning_count warnings" : '');
    #eval {
    #    $stats .= "\n  total subscriber records: " .
    #        NGCP::BulkProcessor::Projects::Migration::Teletek::Dao::import::Subscriber::countby_subscribernumber() . ' rows';
...
sub import_truncate_flatfile_task {
