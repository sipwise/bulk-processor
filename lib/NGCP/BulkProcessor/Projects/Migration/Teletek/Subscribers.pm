    ...
    if ($context->{bill_subscriber}->{id}) {
        ...
        if (defined $context->{voip_numbers}->{primary}) {
            ...
            $context->{voip_numbers}->{primary}->{id} = NGCP::BulkProcessor::Dao::Trunk::billing::voip_numbers::insert_row($context->{db},
                cc => $context->{numbers}->{primary}->{cc},
                ac => $context->{numbers}->{primary}->{ac},
                sn => $context->{numbers}->{primary}->{sn},
                reseller_id => $context->{reseller}->{id},
                subscriber_id => $context->{bill_subscriber}->{id},
            );
        }
        ...
        $context->{preferences}->{cli}->{id} = set_preference($context,
            ...
            $context->{numbers}->{primary}->{number});
        ...
        $context->{aliases}->{primary}->{id} = NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_dbaliases::insert_row($context->{db},
            domain_id => $context->{prov_subscriber}->{domain_id}, #$context->{provisioning_voip_domain}->{id},
            subscriber_id => $context->{prov_subscriber}->{id}, #$context->{provisioning_subscriber_id},
            username => $context->{numbers}->{primary}->{number},
        );

        $context->{preferences}->{allowed_clis}->{id} = set_preference($context,
            $context->{prov_subscriber}->{id},
        ...
        $result &= NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_dbaliases::delete_dbaliases($context->{db},
            $context->{prov_subscriber}->{id},{ 'NOT IN' => $context->{numbers}->{primary}->{number} });
        ...
        if (length($context->{numbers}->{primary}->{ac}) > 0) {
            $context->{preferences}->{ac}->{id} = set_preference($context,
                $context->{prov_subscriber}->{id},
                $context->{attributes}->{ac},
                $context->{numbers}->{primary}->{ac});
        }
        if (length($context->{numbers}->{primary}->{cc}) > 0) {
...
#    }
#
#    push @nums, $priamrynumber->id;
#
#        $result &= NGCP::BulkProcessor::Dao::Trunk::billing::voip_numbers::release_subscriber_numbers($context->{db},
#            $context->{bill_subscriber}->{id},{ 'NOT IN' => \@nums} });
#
#
#    my @numbers = map { ($_->{cc} . // '') . ($_->{ac} . // '') . ($_->{sn} . // ''); }
#        @{voip_numbers::findby_subscriberid($context->{db},$context->{bill_subscriber}->{id})};
#    delete_dbaliases($context->{db},$context->{prov_subscriber}->{id},);
#if($prov_subs) {
...
sub _provision_susbcriber_init_context {
    ...
    $context->{voip_numbers}->{other} = undef;
    $context->{aliases}->{primary} = undef;
    $context->{aliases}->{other} = undef;

    $context->{preferences} = {};
    #$context->{aliases}->{other} = sort_by_configs(\@numbers,[
    #    {   numeric     => 1,
    #        dir         => 1, #-1,
    #        memberchain => [ 'sn' ],
    #    },
    #]);
    #$context->{aliases}->{primary} = shift(@{$context->{aliases}->{other}});
    ##my $cli
