...
sub _provision_susbcriber {
    ...
    eval {
        ...
        if ((scalar @$existing_billing_voip_subscribers) == 0) {
            ...
            _warn($context,"$context->{sip_username}: " . (scalar @$existing_billing_voip_subscribers) . ' existing billing subscribers found, skipping');
        }
    ...
    if ($err) {
        ...
        if ($skip_errors) {
            _warn($context,"$context->{sip_username}: $err");
        } else {
            _error($context,"$context->{sip_username}: $err");
        }
    }
        ...
        foreach my $domain_billingprofilename_resellername (@$domain_billingprofilename_resellernames) {
            ...
            if (not exists $context->{domain_map}->{$domain}) {
                ...
                if ($@ or not $context->{domain_map}->{$domain}) {
                    ...
                    eval {
                        $context->{domain_map}->{$domain}->{provisioning} =
                            NGCP::BulkProcessor::Dao::Trunk::provisioning::voip_domains::findby_domain($domain);
                    };
                    if ($@ or not $context->{domain_map}->{$domain}->{provisioning}) {
...
sub _provision_susbcriber_init_context {
    ...
    $context->{sip_username} = $first->{sip_username};
    unless (defined $context->{sip_username} and length($context->{sip_username})) {
        _warn($context,"empty sip_username ignored");
        $result = 0;
    }
    $context->{sip_password} = $first->{sip_password};

    $context->{web_username} = $first->{web_username};
    $context->{web_password} = $first->{web_password};
    ...
    foreach my $subscriber (@$subscriber_group) {
        ...
        if (not exists $alias_dupes{$number}) {
            ...
            _warn($context,"$context->{sip_username}: duplicate alias $number ignored");
        ...
        if (not exists $contact_dupes{$subscriber->{contact_hash}}) {
            if (not $context->{contract}) {
                $context->{contract} = {
                    ...
                    contact => {
                        (map { $_ => $subscriber->{$_}; } @NGCP::BulkProcessor::Projects::Migration::Teletek::Dao::import::Subscriber::contact_fieldnames)
                    },
                };
                $contact_dupes{$subscriber->{contact_hash}} = 1;
            } else {
                _warn($context,"$context->{sip_username}: non-unique contact hash, skipped");
                $context->{nonunique_contacts}->{$context->{sip_username}} += 1;
                #$result = 0;
            }
        }
    }
    $context->{aliases} = sort_by_configs(\@aliases,[
        {   numeric     => 1,
            dir         => 1, #-1,
            memberchain => [ 'sn' ],
        },
