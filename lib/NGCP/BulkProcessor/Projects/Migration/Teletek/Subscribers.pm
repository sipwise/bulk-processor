...
sub _provision_susbcriber {
    ...
    return 0 unless _provision_susbcriber_reset_context($context,$subscriber_group);
...
sub _provision_susbcriber_reset_context {

    my ($context,$subscriber_group) = @_;

    my $result = 1;

    $context->{sip_username} = $subscriber_group->[0]->{sip_username};
    $context->{domain} = $subscriber_group->[0]->{domain};
    #$context->{domain} = $subscriber_group->[0]->{domain};
