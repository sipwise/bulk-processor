...
sub _set_call_forward {
    my ($context) = @_;
    _invoke_api($context,\&_set_call_forward);
}
...
sub _set_call_forward {
    ...
    eval {
        my $callforwards;
        if ($dry) {
            $callforwards = NGCP::BulkProcessor::RestRequests::Trunk::Customers::get_item($context->{billing_voip_subscriber}->{id});
        } else {
    ...
    if ($@ or not $result) {
        ...
        _info($context,"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': call forwards ' . $cf_path . ($dry ? ' fetched' : ' set'),1);
    }
    return $result;
...
sub _reset_set_call_forward_context {
    ...
    $context->{call_forwards} = {};

    $context->{call_forwards}->{cfb} = _prepare_callforward($context,$cfb_priorities,$cfb_timeouts,
    ...
    $context->{call_forwards}->{cfu} = _prepare_callforward($context,$cfu_priorities,$cfu_timeouts,
        NGCP::BulkProcessor::Projects::Migration::IPGallery::Dao::import::FeatureOptionSetItem::findby_subscribernumber_option_optionsetitem(
            $context->{cli},
            $NGCP::BulkProcessor::Projects::Migration::IPGallery::Dao::import::FeatureOptionSetItem::FORWARD_ALL_CALLS_OPTION_SET,
    ));

    $context->{call_forwards}->{cft} = _prepare_callforward($context,$cft_priorities,$cft_timeouts,
        NGCP::BulkProcessor::Projects::Migration::IPGallery::Dao::import::FeatureOptionSetItem::findby_subscribernumber_option_optionsetitem(
            $context->{cli},
            $NGCP::BulkProcessor::Projects::Migration::IPGallery::Dao::import::FeatureOptionSetItem::FORWARD_ON_NO_ANSWER_OPTION_SET,
    ));
    $context->{call_forwards}->{cft}->{ringtimeout} = $ringtimeout if defined $context->{call_forwards}->{cft};

    $context->{call_forwards}->{cfna} = _prepare_callforward($context,$cfna_priorities,$cfna_timeouts,
        NGCP::BulkProcessor::Projects::Migration::IPGallery::Dao::import::FeatureOptionSetItem::findby_subscribernumber_option_optionsetitem(
            $context->{cli},
