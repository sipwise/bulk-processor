...
sub _create_lnp {
    ...
    #    $context->{db}->db_begin();
        ...
        if ((scalar @$existing_lnp_number) == 0) {
            if ($context->{delta} eq
        ...
        } elsif ((scalar @$existing_lnp_number) >= 1) {
            _warn($context,"($context->{rownum}) " . 'multiple (' . (scalar @$existing_lnp_number) . ') existing lnp\'s ' . $context->{number} . ' found, processing each') if ((scalar @$existing_lnp_number) > 1);
            foreach my $existing_lnp_number (@$existing_lnp_number) {
