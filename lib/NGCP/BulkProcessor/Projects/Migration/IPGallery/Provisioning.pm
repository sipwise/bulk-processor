    ...
    return $result && NGCP::BulkProcessor::Projects::Migration::IPGallery::Dao::import::Subscriber::process_records(
        process_code => sub {
            ...
            foreach my $imported_subscriber (@$records) {
                $rownum++;
                next unless _reset_context($context,$imported_subscriber,$rownum);
                ...
                eval {
                    ...
                    if ((scalar @$existing_billing_voip_subscribers) == 0) {
                        ...
                        if ($imported_subscriber->{delta} eq
                            ...
                            processing_info($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ' is deleted, and no active subscriber found',getlogger(__PACKAGE__));
                        } else {
                            ...
                            if (not defined $existing_provisioning_voip_dbalias) {
                                ...
                                processing_info($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ' successfully provisioned' ,getlogger(__PACKAGE__),1);


                            } else {
                                processing_info($context->{tid},"($context->{rownum}) " . 'existing provisioning voip_dbalias with username ' . $context->{cli} . ' found, skipping' ,getlogger(__PACKAGE__));
                            }
                        }
                        ...
                        if ($imported_subscriber->{delta} eq
                            ...
                            if ($context->{userpassworddelta} eq
                                ...
                                processing_info($context->{tid},"($context->{rownum}) " . 'existing billing subscriber with username ' . $context->{username} . ' and updated password found (re-provisioned)' ,getlogger(__PACKAGE__));
                                ...
                                if (_terminate_subscriber($context,$existing_billing_voip_subscriber->{id})) {
                                    if (_terminate_contract($context,$existing_billing_voip_subscriber->{contract_id})) {
                                        if ($dry) {
                                            ...
                                            if ($@) {
                                                processing_info($context->{tid},"($context->{rownum}) " . 'expected error ' . $@ . ' while re-provisioning subscriber ' . $context->{cli} . ' in dry-mode' ,getlogger(__PACKAGE__));
                                            } else {
                                                if ($skip_errors) {
                                                    rowprocessingwarn($context->{tid},"($context->{rownum}) " . 'expected error while re-provisioning subscriber ' . $context->{cli} . ' in dry-mode missing',getlogger(__PACKAGE__));
                                                } else {
                                                    rowprocessingerror($context->{tid},"($context->{rownum}) " . 'expected error while re-provisioning subscriber ' . $context->{cli} . ' in dry-mode missing',getlogger(__PACKAGE__));
                                                }
                                            }
                                        } else {
                                            _create_contact($context);
                                            _create_contract($context);
                                            _create_subscriber($context);
                                            processing_info($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ' successfully re-provisioned' ,getlogger(__PACKAGE__));
                                        }
                                    }
                                }
                            } else {
                                processing_info($context->{tid},"($context->{rownum}) " . 'existing billing subscriber with username ' . $context->{username} . ' and unchanged password found, skipping' ,getlogger(__PACKAGE__),1);
                            }
                        }
                    } else {
...
sub _reset_context {
    ...
    if (defined $userpassword) {
        ...
        if ($imported_subscriber->{delta} eq
            ...
            if ($skip_errors) {
                # for now, as username+passwords are incomplete:
                rowprocessingwarn($context->{tid},"($context->{rownum}) " . 'no username/password for subscriber found: ' . $context->{cli},getlogger(__PACKAGE__));
            } else {
                rowprocessingerror($context->{tid},"($context->{rownum}) " . 'no username/password for subscriber found: ' . $context->{cli},getlogger(__PACKAGE__));
            }
        }
    }
...
sub _terminate_subscriber {
    ...
    if ($@ or not $result) {
        if ($skip_errors) {
            rowprocessingwarn($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': could not ' . ($dry ? 'fetch' : 'terminate') . ' old subscriber ' . $subscriber_path,getlogger(__PACKAGE__));
        } else {
            rowprocessingerror($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': could not ' . ($dry ? 'fetch' : 'terminate') . ' old subscriber ' . $subscriber_path,getlogger(__PACKAGE__));
        }
    } else {
        processing_info($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': old subscriber ' . $subscriber_path . ($dry ? ' fetched' : ' terminated'),getlogger(__PACKAGE__));
    }
    return $result;
...
sub _terminate_contract {
    ...
    if ($@ or not $result) {
        if ($skip_errors) {
            rowprocessingwarn($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': could not ' . ($dry ? 'fetch' : 'terminate') . ' old contract ' . $contract_path,getlogger(__PACKAGE__));
        } else {
            rowprocessingerror($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': could not ' . ($dry ? 'fetch' : 'terminate') . ' old contract ' . $contract_path,getlogger(__PACKAGE__));
        }
    } else {
        processing_info($context->{tid},"($context->{rownum}) " . 'subscriber ' . $context->{cli} . ': old contract ' . $contract_path . ($dry ? ' fetched' : ' terminated'),getlogger(__PACKAGE__));
    }
    return $result;
