...
sub _create_subscriber {
    ...
    $context->{account_id_preference_id} = set_preference($context,$context->{provisioning_subscriber_id},$context->{account_id_attribute},$context->{cli});
    if (defined $context->{e164}->{ac} and length($context->{e164}->{ac}) > 0) {
        $context->{ac_preference_id} = set_preference($context,$context->{provisioning_subscriber_id},$context->{ac_attribute},$context->{cli});
    }
    $context->{cc_preference_id} = set_preference($context,$context->{provisioning_subscriber_id},$context->{cc_attribute},$context->{cli});
...
sub _reset_context {
    ...
    delete $context->{account_id_attribute};
    delete $context->{ac_attribute};
    delete $context->{cc_attribute};
