...
sub countby_subscribernumber_option_optionsetitem {
    ...
    my ($subscribernumber,$options,$optionsetitem) = @_;
    ...
    if (defined $subscribernumber) {
        $stmt .= ' WHERE ' . $db->columnidentifier('subscribernumber') . ' = ?';
        push(@params,$subscribernumber);
        if (defined $options and 'HASH' eq ref $options) {
            foreach my $in (keys %$options) {
                my @values = (defined $options->{$in} and 'ARRAY' eq ref $options->{$in} ? @{$options->{$in}} : ($options->{$in}));
                $stmt .= ' AND ' . $db->columnidentifier('option') . ' ' . $in . ' (' . substr(',?' x scalar @values,1) . ')';
                push(@params,@values);
            }
            if (defined $optionsetitem and length($optionsetitem) > 0) {
                $stmt .= ' AND ' . $db->columnidentifier('optionsetitem') . ' = ?';
        ...
        } elsif (defined $options and length($options) > 0) {
            $stmt .= ' AND ' . $db->columnidentifier('option') . ' = ?';
            push(@params,$options);
            if (defined $optionsetitem and length($optionsetitem) > 0) {
                $stmt .= ' AND ' . $db->columnidentifier('optionsetitem') . ' = ?';
