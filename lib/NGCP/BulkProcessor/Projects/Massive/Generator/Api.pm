...
sub _load_provider {
    ...
    $result &= defined $provider->{reseller};

    $provider->{contract} = NGCP::BulkProcessor::RestRequests::Trunk::Contracts::get_item($provider->{reseller}->{contract_id}) if $result;
    $result &= defined $provider->{contract};

    $provider->{contact} = NGCP::BulkProcessor::RestRequests::Trunk::SystemContacts::get_item($provider->{contract}->{contact_id}) if $result;
    $result &= defined $provider->{contact};

    $provider->{domain} = _find_entity('NGCP::BulkProcessor::RestRequests::Trunk::Domains',
		domain => $domain_name,
	) if $result;
    $result &= defined $provider->{domain};

    if ($result) {
        my $provider_profile;
        $provider_profile = NGCP::BulkProcessor::RestRequests::Trunk::BillingProfiles::get_item($provider->{contract}->{billing_profile_id}) if $result;
