...
sub reset_fsn {
    ...
    eval {
        ...
        $file_sequence_number = NGCP::BulkProcessor::Dao::Trunk::accounting::mark::get_system_mark(undef,
            $export_cdr_stream
        ); #load mark...
    };
    if ($@) {
        ...
        _info($context,"file sequence number reset to $file_sequence_number")
    }
    return $result;
