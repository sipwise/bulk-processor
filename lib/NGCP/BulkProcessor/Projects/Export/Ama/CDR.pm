...
sub _commit_export_status {
    ...
    my (
        ...
        context
    /};
    _info($context,"file " . $context->{file}->get_filename() . " (" . kbytes2gigs(int((-s $context->{file}->get_filename()) / 1024)) . ") written (" . $context->{file}->{record_count} . " records in " . (scalar @{$context->{file}->{blocks}}) . " blocks)");
    eval {
        ping_dbs();
        $context->{db}->db_begin();
