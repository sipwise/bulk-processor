...
sub _ua_request {
    ...
    if ($@) {
        restrequesterror('error executing rest request: ' . $@,$req,$self->{requestdata},$logger);
    }
    return $res;
}
...
sub post {
    ...
    if ($@) {
        restrequesterror('error encoding POST request content: ' . $@,$self->{req},$data,$logger);
    }
    ...
    if ($@) {
        restresponseerror('error decoding POST response content: ' . $@,$self->{res},$logger);
    }
	return $self->{res};
...
sub get {
    ...
    if ($@) {
        restresponseerror('error decoding GET response content: ' . $@,$self->{res},$logger);
    }	
	return $self->{res};
...
sub patch {
    ...
    if ($@) {
        restrequesterror('error encoding PATCH request content: ' . $@,$self->{req},$data,$logger);
    }
	$self->{res} = $self->_ua_request($self->{req});
	eval {
    ...
    if ($@) {
        restresponseerror('error decoding PATCH response content: ' . $@,$self->{res},$logger);
    }
	return $self->{res};
...
sub put {
    ...
    if ($@) {
        restrequesterror('error encoding PUT request content: ' . $@,$self->{req},$data,$logger);
    }
	$self->{res} = $self->_ua_request($self->{req});
	eval {
    ...
    if ($@) {
        restresponseerror('error decoding PUT response content: ' . $@,$self->{res},$logger);
    }
	return $self->{res};
...
sub del {
	...
	_add_delete_headers($self->{req});
	$self->{res} = $self->_ua_request($self->{req});
	eval {
        $self->{responsedata} = $self->_decode_delete_response($self->{res}->decoded_content());
    };
    if ($@) {
        restresponseerror('error decoding DELETE response content: ' . $@,$self->{res},$logger);
    }
	return $self->{res};
